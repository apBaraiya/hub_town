...
/**
 * Scene2
 * Displays public/models/scene_2.glb with smooth scroll animation, 
 * idle motion, and fade transitions.
 *
 * @param {{ scale: number }} props
 */
export default function Scene2({ scale = 1 }) {
  const { scene } = useGLTF('/models/scene_2.glb')
  ...
  useEffect(() => {
    if (scene) {
      console.log('Scene 2 hierarchy:', scene)
  ...
  useFrame((state, delta) => {
    ...
    const rotationY = easedP * -0.6
    const positionX = easedP * 0.5
    const scaleVal = 1 + easedP * 0.08
