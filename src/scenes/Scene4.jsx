...
/**
 * Scene4
 * Displays public/models/scene_4.glb with smooth scroll animation, 
 * idle motion, and fade transitions.
 *
 * @param {{ scale: number }} props
 */
export default function Scene4({ scale = 1 }) {
  const { scene } = useGLTF('/models/scene_4.glb')
  ...
  // For the final scene, make the boundary inclusive of 1.00
  ...
  useEffect(() => {
    if (scene) {
      console.log('Scene 4 hierarchy:', scene)
  ...
  useFrame((state, delta) => {
    ...
    const rotationY = easedP * -1.0
    const positionY = easedP * 0.4
    const scaleVal = 1 + easedP * 0.12
